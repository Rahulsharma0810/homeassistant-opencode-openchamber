...
- **Optional startup script** — a new opt-in `startup_script` option runs a user-provided shell script on every start, before the main OpenCode services. Use it to install extra third-party npm packages (for example the [skillshare](https://github.com/runkids/skillshare) CLI) or run git commands to fetch skills/config into the persistent `/data` volume. Anything installed with `npm install -g` persists across restarts and updates because the npm global prefix lives under `/data`. Configured environment variables are available to the script, so tokens for private repositories can be supplied via the Environment variables option. When `startup_script` is empty the feature is completely inert; a non-zero exit is logged as a warning and never blocks the add-on from starting.
