...
| **Startup script** | `""` | Optional shell script run on every start, before the main OpenCode services. Use it to install extra npm packages or fetch skills/config with git. See [Startup Script](#startup-script). |

### Startup Script

The **Startup script** option runs a shell script you provide on every start, before the main OpenCode services. It is the general-purpose escape hatch for extending the runtime — installing extra third-party npm packages or fetching skills/config with git — without rebuilding the add-on.

Key properties:

- **Persistence.** The script runs with the same environment as OpenCode, including the npm global prefix under `/data` (`/data/.npm-global`). Anything installed with `npm install -g <pkg>` therefore persists across restarts and updates. Files written under `/data` (for example `/data/.config/opencode/skills`) also persist.
- **Environment variables.** Variables you define in the **Environment variables** option are available to the script, so tokens or credentials (for example for a private git repository) can be supplied there rather than hard-coded in the script.
- **Non-blocking.** A non-zero exit is logged as a warning but never blocks the add-on from starting.

#### Example: install skillshare and sync a skills repository

This installs the [skillshare](https://github.com/runkids/skillshare) CLI and clones a skills repository into OpenCode's global skills directory. It is idempotent — it clones on first run and pulls on later runs:

```bash
# Install the skillshare CLI (persists under /data across restarts).
npm install -g skillshare

# Clone on first run, fast-forward on later runs.
REPO="https://github.com/you/skillshare-skills.git"
DEST="/data/.config/skillshare"
if [ -d "${DEST}/.git" ]; then
...
# Copy the repo's skills/ into OpenCode's global skills directory.
mkdir -p /data/.config/opencode/skills
cp -a "${DEST}/skills/." /data/.config/opencode/skills/
```

Notes:

- **Idempotency.** The script runs on every start, so guard one-time work (like the clone-vs-pull check above) to keep restarts fast and predictable.
- **Security.** Do not paste secrets directly into the script. Put tokens in the **Environment variables** option and reference them (for example `git clone "https://oauth2:${GIT_TOKEN}@github.com/you/private.git"`). The script file is written with restrictive permissions and removed after it runs, and its contents are not passed on the command line.
- **For a private repository**, create a fine-grained token limited to **Contents: read** on that single repository, store it as an environment variable, and reference it from the script.
