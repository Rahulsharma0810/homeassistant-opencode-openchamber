...
- **Existing V2 state upgrades**: State created by the previously shipped V2 builds (`beta-18684` and `beta-19242`) is upgraded using a private copy, preserving conversations, provider sign-ins, and session permissions. The original generation is retained for recovery. Failed upgrades keep the original selected; choose **OpenCode runtime: V1** explicitly if rollback is needed rather than deleting `/data/v2`.
