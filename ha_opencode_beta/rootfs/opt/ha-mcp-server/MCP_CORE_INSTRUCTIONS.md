...
- To assign a device or entity to a room, use `get_areas` and `get_devices`/`get_entity_details` for exact IDs, then request approval before `set_device_area` or `set_entity_area` (full profile). An entity area is an explicit override; clearing it makes the entity inherit its device's area.
